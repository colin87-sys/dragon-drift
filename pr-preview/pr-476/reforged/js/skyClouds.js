...
export const CLOUD_HEAD = /* glsl */`
  ...
  // Cloud-coverage read for the god-ray "negative of the clouds" gap-field. The SPATIAL registration
  // (cuv projection, drift, warp, noise n, vertical band) is IDENTICAL to CLOUD_BODY so the carve sits
  // pixel-for-pixel on the visible deck — but the shape→coverage ramp is DELIBERATELY WIDER than the sky's
  // (0.40,0.72). The sky wants crisp cloud cores; the LIGHT field wants feathered edges so the shafts read
  // as penumbra, not defined beams. The sky does NOT call this (CLOUD_BODY stays inline, unchanged), so the
  // sky render is byte-identical — this is a pure addition, dead-code-eliminated wherever unused. time,
  // lo and hi are params (not uniforms) so CLOUD_HEAD adds no new sky uniform. Keep the registration
  // (cuv/warp/n/band) IN LOCKSTEP with CLOUD_BODY; only the lo/hi shape ramp is a free light-field knob.
  float _cloudCov(vec3 d, float h, float time, float lo, float hi){
