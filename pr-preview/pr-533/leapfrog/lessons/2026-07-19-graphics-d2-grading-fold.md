...
- **The fold is free in look and real in bandwidth.** Pixel A/B of the SAME frozen frame
  through both chains **at aberration 0**: maxΔ = 1/255 with ~94% of pixels
  byte-identical (900×640 hub) — the residual 1-LSB flips are the *removed* HalfFloat
  quantize of the old intermediate RT, i.e. the fused path is sub-LSB MORE precise. On
  the old chain the `OutputPass` hop wrote into a 4×-multisampled RT — so the fold
  deletes a full-frame MSAA store+resolve+reload, not just a texture read.
- **A fold that removes an intermediate RT can't be byte-exact where a later pass FILTERED
  that intermediate — record the deviation, don't overclaim it (Gate-2 catch).** The
  shipped chain bilinear-filtered the *already-encoded* intermediate at the 3
  chromatic-aberration offsets (encode→filter). The fold removed that intermediate, so
  the fused pass filters *linear HDR then encodes* per tap (filter→encode). Since encode
  is non-linear, `tonemap(lerp(a,b)) ≠ lerp(tonemap(a),tonemap(b))` at sub-texel offsets
  over HDR edges → the two DIVERGE once CA is on: up to ~105/255 on ~0.1–0.3% of pixels
  (transient glint edges; meanΔ ≈ 0.07 LSB — edge-local, not a global cast). It's
  inherent (no encoded intermediate left to sample) and arguably the more physically
  correct dispersion — but "byte-target-identical, never a math change" was an
  **overclaim**, and the pixel A/B + the guard both froze frames at ab≈0, structurally
  blind to it. THE LESSON: when a fold changes *when* a nonlinearity is applied relative
  to a filter, your identity claim is only as strong as the regime your probe samples —
  force the effect that exercises the changed order (here, aberration) or your guard is
  measuring the one case that can't fail. Fix: claim corrected to a recorded justified
  deviation; `gradefold.mjs` extended with a forced-aberration A/B (ab 0 ≤1 LSB; ab
  0.025 → >8-LSB <1%, meanΔ <1 LSB); owner CA-active still for the preview. Each of the
  3 taps is still encoded individually as a full vec3 (ACES is a cross-channel fit).
