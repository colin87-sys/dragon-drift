...
### L208 — BRINEHOLM feel-fix (owner playtest): a lance target you must fly into the wall to reach is a dead verb — pull it in, and add a QUIET breach beat (texture without pressure)

**Owner report on the shipped slot-8 breather:** "boring, he doesn't do much" + "his side organs are hard to paint, I crash into the side wall trying and die" + "I unlocked more missiles but there's only ~3 paintable targets." All three were real, and the `lockdps` tool had already quantified the third (BRINEHOLM `5/5/5/2` — cap-5 only reachable by committing to the side shackles + stacking the eye). Two changes, both def/model-gated (roster byte-identical):
...
**2. Added a QUIET breach beat (the anti-boring fix that doesn't add difficulty).** The engine had no per-boss "signature move" hook — texture comes from `phases[].attacks` (pure data) or a `SETPIECE_PATHS` entry keyed by a `def.setpieces[].id`. A NON-MOVING setpiece auto-suppresses fire (`armSetpieceForPhase` pushes attack/rider timers), and the §5f eye-weak-point drive surfaces the eye whenever `chargeT===0` — so a non-moving beat is a fire-QUIET window with the weak-point OPEN, for free. Added `SETPIECE_PATHS.swell` (the island heaves up, leans in, holds reared, settles — the signature breach frame) + `{ id:'swell', atPhase:1, dur:4.5, moving:false }`. Zero model code: `dreadK` (the submerge) is already gated on `sdef.dread`, and `setpieceK` is assigned-but-never-read, so a non-dread setpiece produces the pose-path rise and nothing else. **Result: a memorable beat + a SAFE paint window on the relief boss, adding readability, not bullets.** Lesson: **on a boss that must stay easy, the cheapest "more interesting" is a telegraphed PAUSE with the weak-point exposed — spectacle and a fair reward, negative pressure.**

**Verify.** boss.mjs 70 green (new asserts: outer shackle |x|<8 with >4 m wall margin; the quiet P2 swell + the moving P4 sounding both present); lockdps/defs/bossboot/smoke/gold-det/bossrush/tricount green; lockdps BRINEHOLM row unchanged (target count didn't move). bossgate G3 fails IDENTICALLY on clean master (~2% danger-band pixels in this headless software-GL — a pre-existing environmental flake; mine is marginally cleaner at 1.83%), so no visual regression — the owner eye-judges the breach's motion/feel + the safer shackle reach on the PR preview.
