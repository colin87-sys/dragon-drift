...
## L247 — UNLEASH PHRASE PR-A: beat-lock via the queue you already have; presentation clocks are free, damage clocks are LAW; a riser must be built cancellable (stepped points, not one long ramp)
...
## L248 — Skill-expression LAW: never auto-snap what the player's timing earns; a playtest LAB is two gated lines when damage is the root of all state change; and a flag-gated A/B must gate BOTH the audio AND the visual it swaps
