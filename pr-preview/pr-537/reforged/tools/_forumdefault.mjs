...
// No hero pin (so every forum archetype spawns). ?biome=0&debug
//   node tools/_forumscene.mjs [tag]  →  reforged-captures/forumdefault-<tag>-*.png
...
for (let i = 0; i < 6 && shot < 6; i++) {
  ...
  writeFileSync(`reforged-captures/forumdefault-${tag}-${String(shot).padStart(2, '0')}.png`, await page.screenshot());
