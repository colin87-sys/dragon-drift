...
for (const a of data) {
  ...
  let innerMin = Infinity, worstTop = 0;
  // Authored-orientation props (paired hero = sungate) always turn their small
  // gap-facing side (xMax) toward the lane, NOT the full random-rotY ρ.
  const facing = a.paired ? a.xMax : a.rho;
  for (const s of a.samples) {
    const top = s.h * a.yMax;
    const lean = top * Math.abs(s.tilt || 0);
  ...
  rows.push({ name: a.name, biomes: a.biomes.join(','), rho: facing.toFixed(3), inner: innerMin.toFixed(1), top: worstTop.toFixed(1), exempt, bad, inScope, overGate: !exempt && innerMin < GATE_VEIL });
}
...
for (const r of rows) {
  const note = r.exempt ? 'exempt (below lane)'
    ...
    : r.overGate ? '\x1b[33mwarn — within gate veil\x1b[0m' : 'ok';
