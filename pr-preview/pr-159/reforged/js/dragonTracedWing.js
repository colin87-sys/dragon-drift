...
function buildTracedWings(def, model, attach) {
  ...
  function side(s) {
    ...
    // SHOULDER-MOUNTED side-profile pose: dihedral raises the wing; sweepBack leans the
    // tips toward the tail. The root stays compact at the shoulder (NOT spread down the
    // spine). The root chord is flattened in the geometry so it seats on the shoulder.
    pivot.rotation.order = 'ZYX';
    pivot.rotation.z = s * (model.wingDihedral ?? 0.85);
    pivot.rotation.y = -s * (model.wingSweepBack ?? 0.35);
    pivot.rotation.x = model.wingPitch ?? 0.05;
    const L = buildLocalWing(); const SC = 1.28 * ws;
    pivot.add(makeWing(s, ws, membraneMat, boneMat));
