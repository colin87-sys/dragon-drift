...
- **Phase 0 — Foundations & free wins (do first):** N2 → N1 → N3 scaffolding → N3 decision → N4. Nothing changes
  ...
  - **✓ Landed — N10b water depth (WATER DEPTH toggle, default OFF).** Fake Beer–Lambert: the water body colour
    mixes deep↔shallow by the view-ray slant path (`trans = exp(-uAbsorbK/max(V.y,0.05))`) instead of by wave
    height, so the sea reads as having volume (dark deeps, bright look-down). Gated in the mix-factor domain
    (`mix(tH, trans, uAbsorbOn)`, identity off); `uAbsorbK` derived per biome in `setWaterTint` from the lerped
    deep/shallow luminance (no new biome fields). ~6 ALU, runs on every tier (incl. flat tier2). Live uniform
    flip (no rebuild). Verified: `tests/water.mjs` 24/24, `?depth` boots clean. **N10 c/d** (foam collars,
    reflection blur/half-rate) remain.
  - **✓ Landed — N10c foam collars (SHORE FOAM toggle, default OFF).** Broken, pulsing foam rings at each prop's
    waterline weld the towers/crystals/ruins into the now-moving sea. New `js/propFoam.js`: one shared ring
    geometry + ShaderMaterial; each prop band gets a **sibling InstancedMesh** whose foam matrix is written at the
    same index in `writeMatrix`, so recycling / biome parking / the arena gate are inherited in lockstep. Rides
    the swell in the vertex shader (shared `SWELL`); layer 1 (skips the god-ray mask + reflector); per-archetype
    `foam:{r}` config (archruin/slab **now get elliptical `{rx,rz}` collars** — a per-axis ring hugs the thin
    footprint; owner-playtest round 2); tilt shifts the pierce point. tier0/1 only. Verified:
    `tests/propfoam.mjs` 18/18, `?foam` boots clean. **N10d** (reflection blur/half-rate) is the last water rung.
  - **✓ Landed — N11 pass-budget: tier1 keeps the wow (no toggle — an adaptive-tier uplift).** God-rays +
    water reflection were **tier-0 only**, so mid-range phones (tier1 = most players) lost the signature look.
    N11 cheapens both so tier1 keeps them, tier0/tier2 **byte-identical**: god-ray mask → quarter-res at tier1
    (`godrays.js setGodRayMaskScale`, 0.5 tier0 / 0.25 tier1, +96px/axis floor for small viewports) + 24 samples
    + halved intensity (`postfx.js _grTierOK = tier<=1`, `_grIntenScale`); reflection → tier-parameterized
    `setWaterReflective(tier)` rebuild (768² tier0 / **384² tier1**, the `mirrorRes` half of the seam key forces
    the tier0↔tier1 rebuild), **half-rate** at tier1 (mirror every other frame via the existing `_reflectSuspended`
    hook + a per-frame parity counter — stale RT *and* stale textureMatrix = no swim), and a **far-plane clamp**
    to `fogFar+50` on both reflective tiers (clamp the MAIN camera around the Reflector's `onBeforeRender`, since
    it copies the main projection; `?reflfar=0` kills it). Verified: `tests/passbudget.mjs` 19/19 (tier truth
    table), `tests/water.mjs` 24/24 (tier-arg signature), tier0 boots 768²+god-rays / tier1 boots 384²+god-rays,
    0 console errors. **N10d** (roughness-blurred reflection) is the follow-up if half-rate judder shows on device.
...
| N11 | N11 pass-budget (tier1 uplift) | Gate-1 ADJUST(7) | built to spec | Gate-1 caught: wrong test target (`composer.mjs` is the AUDIO test → new `tests/passbudget.mjs`); mask SCALE must be per-tier not global 0.5→0.25 (+96px floor — occRT is CSS-px, quarter-res on a 390px phone = ~98px → thin-occluder dropout); the rebuild seam key needs `mirrorRes` (tier0↔tier1 are both reflective → keying on `reflective` alone skips the rebuild); far-clamp goes on the MAIN camera (Reflector copies its projection, not the mirror cam's) reading LIVE material fogFar; half-rate tier1-only (judged vs no-reflection = pure uplift) + parity per-frame not per-draw + keep the stale textureMatrix. tier0/tier2 byte-identical (uSamples 40/×1/mask 0.5/768 full-rate; tier2 composer-off + cheap quad). Owner judges tier1 shaft banding + half-rate judder on the pinned-tier1 preview; `?reflfar=0` A/B. Tests: passbudget 19/19, water 24/24 |
| N10c | N10c foam collars | 8/10 | SHIP | Gate-1 ADJUST+3 (per-band sibling not a 48-pool — there are ~1,050 prop instances; per-archetype foam cfg + tilt pierce; layer 1). Gate-2 no correctness defects — verified identity-off, parking lockstep (foam derived from band.data[i] at the same index, can't desync), swell-ride parity (sag <0.05m lift), layer-1 exclusions from the god-ray mask + reflector, the tilt-pierce math exact. Made VISUAL sign-off a merge condition — the first foam read too faint at prop distances; boosted to near-opaque two-scale churn + a closer montage vantage. Polish: skip degenerate opt-out draws; SWELL int-literal note. Lesson: sibling-instanced decal in lockstep; the code gate can't score intensity — the montage does |
| N10b | N10b water depth | 8/10 | SHIP | Gate-1 GREENLIGHT+3 (t-domain gate; fold to one uAbsorbK; name it `trans`), all built. Gate-2 no code defects — independently verified identity-off (`mix(tH,trans,0)=tH` exact), monotonic bright→dark by V.y, per-biome K well-behaved across all 6 biomes (murk ∈ [0.6,0.83]), triple-redundant uniform survival across the rebuild seams. Two reusable patterns banked: gate in the mix-FACTOR domain (one scalar mix = free exact identity); derive per-biome params from already-lerped colours (free seam smoothness, no new fields). Montage delta: fixed the warp-shot tool (freeze-at-warp so far cells don't drop to the menu) |
