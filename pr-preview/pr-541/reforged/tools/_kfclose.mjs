...
//   node tools/_kfclose.mjs [tag] [camDistMul]  →  reforged-captures/kfclose-<tag>-*.png
...
for (let i = 0; i < 4 && shot < 3; i++) {
  ...
  writeFileSync(`reforged-captures/kfclose-${tag}-${String(shot).padStart(2, '0')}.png`, await page.screenshot());
  console.log('frame' + shot + ':', JSON.stringify(info));
  shot++;
}
await page.evaluate(() => clearInterval(window.__pin));
await done().catch(() => {});
console.log(`kfclose ${tag}: ${shot} frames; err=${errors.length}`);
