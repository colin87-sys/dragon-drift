...
| **H7 ✅ BUILT** | L | **The Living Gauge I** — the **DRAGON VITALS settings toggle** (GAME section, `saveData.settings.dragonVitals`, default OFF; `?vitals=1` stays as the dev alias). `bondChannel.setVitals()` seam (`js/dragonBond.js`, written per-frame from main.js BEFORE updateDragon — frame-0, consumed inside the existing updateDragon material pass, no second rAF); wing-charge stamina (bucket version — 6 charge-stud octahedra on a shape-agnostic shoulder→carpal→tip-marker anchor ladder, 3 rank-shared mats same-mat-L/R, tip drains FIRST so light retreats toward the shoulder; span-gradient `onBeforeCompile` stays deferred); body-light health (multiplier clamp ≥0.75 + ~8%/heart ash lerp) + 12-mote ember-bleed; 1-heart coreGlow danger-magenta heartbeat at ~1.1Hz; `html.vitals-on` deepens the arc rest-ghost 0.30→0.18 **only when ON** | **PASSED 2026-07-16:** flag-OFF proven byte-identical to master — a deterministic off-proof harness drove the real updateDragon 300 scripted steps (azure/vesper/jade): full scene state dumps AND rendered pixel hashes matched exactly, and the harness diverges with the flag on; tricount unchanged (FX are lazy, runtime-only); uitokens/smoke/starters/gauntlet/hudstate green; on-device 60fps rides the preview (`?debug=perf`) |
| **H8 ✅ BUILT** | M | **The Living Gauge II** — same toggle. Spine-ignition surge nodes (5, nose→tail, one per chained ring; all-lit = tail-tip node blazes gold): **the shape-agnostic fallback contract SHIPPED** — authored `userData.vitalsSurgeNode` markers → tail-chain segments → coreGlow steps; trail-as-combo tint (lerp toward ember-gold, **cap 0.5**); +120ms Bond-Echo (dragon frame 0 via the pre-updateDragon seam write; the DOM gem-pop echoes via `html.vitals-on` animation-delay — the lit fill stays frame-0 authoritative); surge-horn rest-ghost deepens with the flag | **PASSED 2026-07-16:** roster fallback proven on ALL 14 unauthored dragons (13 tail-chain, jade = coreGlow fallback — zero errors, tip blaze + directional wing ladder asserted); no roster dragon has authored markers yet — **hero authoring is open debt**; flag-OFF re-proven byte-identical after H8 |
...
- **DRAGON VITALS** (toggle — **SHIPPED with H7/H8, 2026-07-16**, GAME section,
  `saveData.settings.dragonVitals` default OFF, `?vitals=1` dev alias): enables the
  flagged living-gauge channels (wing-charge stamina, body-light health + ember-bleed +
  1-heart heartbeat, spine-ignition surge + trail-as-combo) and deepens the chrome
  rest-ghosts one step (arc + surge horn 0.30→0.18 via `html.vitals-on`; LIFE already
  rests at 0) — Law 1's contract. OFF is proven byte-identical to the shipped dragon.
