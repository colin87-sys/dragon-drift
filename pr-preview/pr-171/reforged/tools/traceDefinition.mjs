...
let boneShapesRaw = [], boneSpinesRaw = [], strutsRaw = [], veinsRaw = [];
...
if (existsSync(boneFile)) {
  ...
  boneSpinesRaw = (J.boneSpines || []).map((s) => ({ pts: s.pts.map((p) => ({ x: p[0], y: p[1] })), radii: s.radii.slice() }));   // centerline + half-width → cylindrical tapered bones
...
const bones = boneSpinesRaw.map((s) => ({ pts: s.pts.map(place), radii: s.radii.map((r) => r * fit) }));   // cylindrical tapered spines (radius scaled by the placement fit)
...
const DEF = {
  ...
  wing: { side: 'right', silhouette: wingSilhouette, struts, boneShapes, bones, veins, sparIndex },
};
...
if (emit) {
  ...
  const data = { canvas: DEF.canvas, source: DEF.source, mirror: DEF.mirror, body: { silhouette: r(DEF.body.silhouette), plates: DEF.body.plates.map(r), head }, wing: { side: DEF.wing.side, silhouette: r(DEF.wing.silhouette), struts: DEF.wing.struts.map(r), boneShapes: DEF.wing.boneShapes.map(r), bones: DEF.wing.bones.map((b) => ({ pts: r(b.pts), radii: b.radii.map((v) => +v.toFixed(5)) })), veins: DEF.wing.veins.map(r), sparIndex: DEF.wing.sparIndex } };
