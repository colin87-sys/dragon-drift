...
### L246 — A playtest shortcut = surface the debug seam you already have in the menu, dev-gated; don't build new plumbing

**Did / learned.** Owner wanted to "select different phases in the gauntlet to quickly playtest." The engine ALREADY had the whole mechanism: `?bossPhase=N` → `setBossDebugPhase(n)` fast-forwards a fight to phase N (hp parked just above that phase's floor + the phase's card/setpiece armed through the live break path). The ask was just to reach it without editing the URL. So the build was tiny: a dev-gated segmented selector on the existing rush/gauntlet picker that arms a phase, and threading that phase through the existing `onStartRush(only, phase)` → `setBossDebugPhase`. No new fight logic.
1. **The rush picker already carried a solo-boss launch (`onStartRush(bossId)`); adding a second arg (the phase) was the whole engine change.** `setBossDebugPhase(phase || 1)` on every launch (phase 1 → null → normal start) means the default path is byte-unchanged and the feature is opt-in by arming P2+.
2. **Dev-gate on the flag the roster already computes.** `rushRosterInfo()` knew `devAll` (the `?dev`/`?rush=all` seam) — expose it + each boss's phase names, and the selector renders only in dev. A player never sees it; a playtester gets it for free on the preview.
3. **Test the PLUMBING cheaply, not the slow end-to-end.** Asserting the fight actually reached phase 2 means polling through a slow apex boss's warn→approach→fight (>12s, flaky). Instead expose the armed jump in `bossDebugState()` (`phaseJump`) and assert it right after launch — proves menu→`setBossDebugPhase` in ~700ms. The application (`phaseIdx = target`) is the pre-existing, unchanged `?bossPhase` path.
4. **A global selector armed before the boss pick can't know the boss, so show the roster MAX phase count (P1–P5) and let `setBossDebugPhase` clamp per boss** (`Math.min(jump, phases.length-1)`) — a 3-phase boss silently uses phase 3 for P4/P5. Fine for a dev tool; simpler than per-chip phase menus.

**Gotcha.** The debug phase-jump is a module-level var read at fight-start and NOT auto-consumed, so it persists across a gauntlet — set it FRESH on every launch (solo → the armed phase; the default → 1/null) so a stale pick doesn't leak into the next run.

**→ Systematize.** (1) "Let me quickly playtest X" almost always = a debug seam that already exists behind a URL param; surface it in the relevant menu, dev-gated, instead of building anything new. (2) Thread the new choice through the existing launch handler as an extra arg that defaults to the shipped behaviour. (3) Gate dev UI on the flag the data layer already exposes (`devAll`). (4) For a UI test, verify the armed intent via a debug-state field, not the slow downstream effect.

**→ Leapfrog.** Boss-rush picker now has a dev phase-jump selector (P1–P5, dev-only) — tap a phase, tap a boss, land in that stage in seconds (THE UNMASKED's stage 2/3 included). `bossrushui.mjs` +4 checks (dev-gating, arm/active, launch, armed-jump), `boss.mjs` 103 green, `bossrush.mjs` 3 green; non-dev roster unchanged. Reusable for any future stage/phase playtesting.
