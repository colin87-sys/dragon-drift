...
ui.init({
  ...
  onStartRush: (only, phase) => {
    rushOnlyBoss = only || null;
    // Dev phase-jump (rush picker, ?dev): open the encounter fast-forwarded to phase N so a
    // late-stage beat is playtestable in seconds. phase 1 (or unset) → null → normal start.
    setBossDebugPhase(phase || 1);
    startGame('rush');
  },  // pick one boss (or all), optionally jumped to a phase
  onEquipDragon: () => {
