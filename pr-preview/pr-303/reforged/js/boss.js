...
export function rushRosterInfo() {
  ...
  return {
    bosses: BOSS_ORDER.map((k) => ({
      ...
      // Phase/stage labels (for the dev phase-jump selector): the def's phase names, so a
      // playtester can jump straight to e.g. THE UNMASKED's stage 2/3 from the picker.
      phases: (BOSSES[k].phases || []).map((p) => p.name || p.id || ''),
    })),
    unlockedCount: rushRoster().length,
    bestClearMs: saveData.bossRush?.bestClearMs || 0,
    cleared: saveData.bossRush?.cleared || 0,
    // Dev seam (?dev / ?rush=all): expose the phase-jump selector for quick playtesting.
    devAll,
  };
}
...
export function bossDebugState() {
  ...
  return { active, phase, hp, hpMax, phaseIdx, shielded, bullets: bossBulletCount(), nextBossDist, warnT, approachT, poseRel: pose.rel, poseX: pose.x, poseY: pose.y, setpiece: setpieceT >= 0, charging: chargeT > 0, chargeLevel, ghostFrameBroken, ghostFrameHits, soakT, phaseJump: debugPhaseJump };
}
