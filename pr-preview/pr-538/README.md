...
- **Rendering**: EffectComposer pipeline (bloom → **occlusion-masked god-rays** → ACES tone map → vibrance/vignette/chromatic-aberration grading), planar-reflection water (real 768² mirror pass on tier 0, analytic fake on lower tiers) with **crest foam + sun-glitter**, a procedural **contact shadow** grounding the dragon on the sea, a fresnel **rim light** that lifts the hero silhouette, and instanced prop bands (with procedural surface micro-noise) recycled around the player. The god-rays render a cheap half-res sky/occluder mask so shafts stream from open sky and are blocked by silhouettes — bright foreground (rings/dragon) never haloes. Everything is procedural — shaders, math and canvas textures, no asset files.
- **Adaptive quality**: 3 tiers driven by FPS with hysteresis (pixel ratio, bloom, god-rays [tier 0 only], reflection, contact shadow, rim strength, particle counts, birds). Override in Settings. `?debug=perf` shows fps/draw-calls/tier.
