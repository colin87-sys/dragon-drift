...
- **4 MARROWCOIL** — skull: box cranium 1.6w + tapered snout box + jawPivot jaw slab + 2
  ...
  FLY-THROUGH → FULL MANEUVER (2026-07 — L151): `ribThread` is now the whole aerobatic over 8s —
  ...
  → CINEMATIC AMBUSH (2026-07 — L152, dur→10s): the pass now flies fully OFF-SCREEN, then a mid-fight
  REAR-LOOK camera (`cameraCtl.setOvertake` works outside the entrance) reveals the boss already turned
  around (yaw π while BEHIND = facing you) accelerating in mouth-open, firing **FROM-BEHIND** bullets
  (free: a boss bullet's direction is the sign of `vrel` — spawn `rel<0`, close `vrel>0`; the three
  bossBullets "behind" cases generalise on that sign, byte-safe roster-wide), then it banks away, the
  camera returns, and on the side lane it TURNS ITS HEAD at you (`setHeadLook` — a wide skull-yaw channel
  added onto the ±9° gaze; aim stays positional) for a last few shots. Player keeps full control (Mantis).
