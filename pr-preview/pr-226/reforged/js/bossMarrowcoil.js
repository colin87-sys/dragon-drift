...
export function buildBoneCoil(def, quality = 1) {
  ...
  // HEAD-TURN (L152): a WIDE skull yaw (radians) beyond the ±9° gaze look — the flank
  ...
  function tickBody(dt, time) {
    ...
    if (Math.abs(headLookTarget) > 0.3 && lookAwayT > 0) lookAwayT = 0;   // mid head-turn, no idle drift (L152)
    ...
    headLook += (headLookTarget - headLook) * Math.min(1, dt * 4.5);   // L152 head-turn ease (a shade snappier than gaze)
    ...
    skull.rotation.y = gazeX * 0.16 + charge * 0.06 + headLook;   // roar yaw kept subtle; +headLook is the L152 wide head-turn (flank look-back)
