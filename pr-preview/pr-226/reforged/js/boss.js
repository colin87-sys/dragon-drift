...
let cineRoll = 0;              // scripted bank (rotation.z) — a setpiece path may return `roll`; 0 = level (L149)
let ribEmitT = 0;             // sub-cadence accumulator for the ribThread rib-bullet emit (L149)
...
const SETPIECE_PATHS = {
  ...
  ribThread(k) {
    const B = CONFIG.BOSS;
    // L149 — the FULL fly-through MANEUVER (was L141/L147's loom→thread, now the whole
    // pass-and-bank). Returns {x,y,rel,yaw,roll}: the runner routes yaw→cineYaw (facing)
    // and roll→cineRoll (bank). Six beats, back turned in the middle so it flies YOUR
    // heading, then banks into the lane and wheels around to face you again:
    //   1 loom     — close from station to the aperture, facing you (yaw = face-player).
    //   2 fly past — sweep rel NEAR→DEEP and DIVE ~4.2u at the thread instant (L147's
    //                barrel-onto-rail axis), sliding to a flank as it clears BEHIND you.
    //   3 turn     — hold the flank, wheel yaw 0→π (back to you, flying your direction).
    //   4 overtake — accelerate rel −2→AHEAD, passing you on the flank, still back-turned.
    //   5 bank in  — curve x→0 with a cineRoll BANK while yaw wheels π→0 (turning to face you).
    //   6 restore  — settle to station (rel→settleGap), level and facing you; clearSetpiece nulls the channels.
    const NEAR = 7, DEEP = -13, DIVE = 4.2, AHEAD = 14, FLANK = 8, side = 1;
    // 1 — loom (facing you: no yaw key → cineYaw null → placeGroup face-player default).
    if (k < 0.18) { const t = easeInOut(k / 0.18); return { x: 0, y: B.fightHeight, rel: B.settleGap + (NEAR - B.settleGap) * t }; }
    // 2 — fly past + dive (still facing you as it threads THROUGH, then clears behind).
    if (k < 0.38) { const t = (k - 0.18) / 0.20, e = easeInOut(t), s = Math.sin(t * Math.PI);
      return { x: side * FLANK * e, y: B.fightHeight - DIVE * s, rel: NEAR + (DEEP - NEAR) * e }; }
    // 3 — back-turned re-approach: wheel yaw 0→π (flying your heading), rise back to frame height.
    if (k < 0.58) { const t = easeInOut((k - 0.38) / 0.20);
      return { x: side * FLANK, y: B.fightHeight, rel: DEEP + (-2 - DEEP) * t, yaw: Math.PI * t, roll: 0 }; }
    // 4 — accelerate past on the flank, still back-turned (yaw held at π).
    if (k < 0.74) { const t = easeInOut((k - 0.58) / 0.16);
      return { x: side * FLANK, y: B.fightHeight, rel: -2 + (AHEAD + 2) * t, yaw: Math.PI, roll: 0 }; }
    // 5 — bank in: curve to the lane, BANK (roll peaks then levels), wheel yaw π→0 to face you.
    if (k < 0.92) { const t = (k - 0.74) / 0.18, e = easeInOut(t), s = Math.sin(t * Math.PI);
      return { x: side * FLANK * (1 - e), y: B.fightHeight, rel: AHEAD + (B.settleGap - AHEAD) * e, yaw: Math.PI * (1 - e), roll: -side * s * 0.5 }; }
    // 6 — restore: hold station, level and facing you (yaw 0); clearSetpiece nulls cineYaw/cineRoll at k≥1.
...
function clearSetpiece() {
  ...
  setpieceT = -1;
  setpieceDef = null;
  cineYaw = null;   // hand facing/banking back to placeGroup's face-player default (L149) —
  cineRoll = 0;     // covers both normal completion (k≥1) and the mid-beat shield abort
  ribEmitT = 0;     // reset the rib-bullet sub-cadence so the next pass starts clean
}
...
export function updateBoss(dt, player, time) {
  ...
  if (phase === 'warn') {
    ...
    if (p && debugSetpiecePin.moveGroup) {
      pose.x = p.x; pose.y = p.y; pose.rel = p.rel;
      // L149: a maneuver pin also holds the path's FACING + BANK so a still shows the
    ...
    if (setpieceT >= 0 && !shielded) {
      ...
      pose.x = p.x; pose.y = p.y; pose.rel = p.rel;
      // A setpiece path MAY drive facing + banking (L149): `yaw` present → cineYaw owns
      ...
      // THE RIB THREAD (L149): the whole pass is its own beat — the boss fires ONLY from
      // inside its ribs. SUPPRESS the normal skull-origin cadence for the ENTIRE ribThread
      // (hold the timer + quietly cancel any in-flight charge) so no skull bullets fire
      // while the head is diving through or turned away, and emit the slow amber converging
      // rib bullets on a sub-cadence WHILE the ribcage straddles the player plane (close band).
...
function placeGroup(player, time, dt) {
  ...
  // 180° to face you, and banks as it curves into the lane — L149).
  ...
  // whenever cineYaw owns facing (L149): at a scripted yaw — the back-turned pass
...
// THREAD-THE-GAP rib emit (L149): during the fly-through pass, a few SLOW, reflectable
