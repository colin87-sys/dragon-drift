...
function measure({ tris, key }) {
  ...
  const profile = []; if(process.env.WR_DUMP){ console.log('FULLCOLS '+key); console.log(cols.filter((_,i)=>i%2===0).map(c=>(c.s*100).toFixed(0)+'|le'+((c.le-cLo)/cMax).toFixed(2)+'/te'+((c.te-cLo)/cMax).toFixed(2)).join(' ')); }
