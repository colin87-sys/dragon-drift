...
// Skip the whole tutorial: mark every gesture taught (so hints.js won't
// re-teach), end the sequence, and resume if we were frozen.
export function skipGestureTutorial() {
  saveData.flags.hintsSeen |= HINT_BIT.steer | HINT_BIT.boost | HINT_BIT.roll | HINT_BIT.phase;
  saveData.flags.seenFirstRoll = true;
  saveData.flags.phaseTaught = true;
  persist();
  const wasPaused = !!paused;
  paused = null; phasePending = false; idx = STEPS.length;
  ui.hideGesture();
  if (wasPaused) cb.onResume();
}
...
export function updateGestureTutorial(player) {
  ...
  baseline = snapshot();
  cb.onPause();
  ui.showGesture({ gesture: step.gesture, touch: isTouch(), text: step.text(), onSkip: skipGestureTutorial });
}
