...
console.log(`  T4 world median drop = ${(wMedSurge / wMedBase).toFixed(3)}× baseline   (aim 0.72–0.82, roster-invariant)`);
console.log(`  TSKY upper-sky drop  = ${(surge.s.sky.medianL / Math.max(base.s.sky.medianL, 1)).toFixed(3)}× baseline   (aim ≤0.60, roster-invariant — color-eye must-fix)`);
