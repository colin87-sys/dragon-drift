...
## L252 — PR-B "make the finale HIT": fold the beat-lock INTO the fuse (a post-action wait reads as lag; a stretched-but-moving one doesn't); a sustained voice needs a release on EVERY path or it drones; impact = a hard front + the chord as decay, and a screen that reacts
...
**Gotchas banked.** (1) Restart the branch from master after a merge (the #302 rule) — master had moved through #303/#304/#305; my ledger renumbered to L252. (2) A short beat-aligned fuse broke T-E2's timing (the cap auto-fired before the test could inject a manual tap) — the new assertion proves the aligned fuse fires SOONER than the plain capFuse (the lag fix, directly) and uses a long fuse for the tap sub-tests. (3) `juice.mjs` (and `badges`) are browser tests that flake at the start-button click in THIS sandbox — confirmed by running them on clean master; the new `wispFinale` juice assertion is well-formed for real CI. (4) Removed now-dead config (`releaseQuantMaxS`/`releaseMinLeadMs`) — grep tests/tools before deleting a config key.
