...
### LANCE/BOSS PROGRESSION AUDIT — turn the lock layer into a curriculum, not a scattered feature

**Did / learned.** Audited the live Reforged Lance system against the current boss roster and wrote `docs/lance-boss-progression-plan.md`. The implementation is already a strong data seam: `virtualLockOrgan` teaches V1 aim-line, `lockParts` names paintable organs, `partWorldPos()` resolves live anatomy, `paintableParts()` handles phase/liveness filters, and Wyrmfire wisps home to branded parts under a hard per-phase ROI clamp. The content issue is sequencing: Voidmaw and Stormrend correctly teach V1, but Ashtalon stays V1-only, so the first real paint lesson lands on Marrowcoil — a busy coiling-rib setpiece with iris/stream/crossfire/moving-gap pressure. Later high-tier bosses also alternate between rich lock anatomy and no lock anatomy without an explicit ladder reason.

**Reusable pattern.** Treat Lance as a boss curriculum: V1 sight-line → V1 under wall pressure → two-target first paint → multi-organ sweep → moving-target exam → destructible anatomy → single-organ timing exception → split weak-point ecology → muted duel/inversion. Lock targets should be organs/wounds/emitters/bindings/trophies, never generic hull; every target needs a fair acquisition window and a liveness rule. A high-tier no-lock boss is valid only when the sheet names the replacement verb (music-lock, beam duel, sealed duel) so it reads as a deliberate inversion instead of regression.
