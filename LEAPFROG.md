...
### L230 — A procedural angel FEATHER-WING is a leading-edge BONE with feathers RAKED off its trailing side — NOT a fan radiating from a point, NOT a spine that runs to the tip (both collapse the area or lose the bend); Fable-gate the silhouette every round

Built ONE isolated procedural low-poly angel wing (`reforged/js/seraphWingShape.js`, `buildSeraphWing(opts)` → group on a shoulder pivot, mirrorable via `scale.x=-1`), driven by an isolated studio (`tools/wingstudio.html` + `tools/wingshot.mjs` → front/profile/3-4 PNG crops) and gated by an independent **harsh Fable critic every round** (reference PNG + build crops in, PASS/FAIL + ranked fixes out). Reached a genuine PASS (5 build rounds to first PASS; confirmed 3× by three independent Fable critics).

**→ The gotcha (the shape kept failing for the SAME reason each time, re-learned three ways):**
- **Feathers pointing ALONG the leading-edge tangent COLLAPSE the wing to a thin diagonal strip** (round 1 v2). A feather rooted on the bone and pointing parallel to it lies *on* the bone — zero area.
- **A fan radiating from ONE point loses the obtuse bend** (it reads as a leaf/hand-fan). **A single spine that runs all the way to the wing-TIP with feathers hanging off it reads as a stick-with-a-fan** (isolated arm + a gap at the wrist).
- The wing that finally reads is **ANATOMICAL**: a near-vertical FOREARM bone S→W (~84°); feathers (coverts + secondaries) RAKE off its trailing side (rake angle WIDE at the base → the wing is broad, not a leaf); at the wrist the PRIMARIES fire off as a **clean FAN** (top blade steepest → the angular jump from the near-vertical forearm to the fan IS the ~120–130° obtuse knuckle, NOT a smooth CatmullRom arc — a rounded corner erases the bend).
- **Primaries: BROAD + overlapping at the BASE, only their ROUNDED TIPS step apart** (length-stagger, not side-gaps). Over-separating them → thin "fingers/tines" (a hard Fable fail); fusing them → one paddle (also a fail). The sweet spot is broad blades whose *tips* stagger into a stepped trailing edge.
- **Profile depth**: flat feather cards seen PURELY edge-on (down the exact span axis) always render as slivers/spikes — that is an artifact of the view, not the model. Fix BOTH: give real front-to-back z-stagger (coverts front → secondaries → primaries back) AND shoot the "profile" ~26–32° off pure-edge-on so the layered shingles read as depth. A per-vertex "global chordwise cup" post-pass BACKFIRED (jagged z-offsets between adjacent feathers → worse spikes) — cup PER FEATHER (camber mapped through the cup-normal) + row z-stagger instead.
- Rounded tips: build the feather lobe outline with a **semicircular tip ARC** (never taper to a point) and a **blunt quill root** (root width ≥ ~34%, not a needle) — the needle roots are what read as "spikes" edge-on.

**→ Systematize.** LAW for a feathered wing: (1) a BONE for the leading edge with a SHARP wrist corner (piecewise-linear, not CatmullRom — smoothing kills the bend); (2) feathers RAKE off the bone's trailing side, rake wide→narrow base→tip; (3) primaries are a tight FAN from the wrist, broad + overlapping bases + length-staggered rounded tips; (4) three z-staggered tiers for depth; (5) judge the SILHOUETTE, and shoot the "profile" a few degrees off pure-edge-on. Harness LAW: to Fable-gate a SHAPE, the builder must render crops to PNG *files* and save the reference to a known path — the spawned Fable agent reads them with the Read tool (which renders images); a fresh independent critic each round (no stake) is harsher and more trustworthy than a continued one. Studio gotcha: `renderer.setSize(w,h,false)` leaves the canvas CSS size at the drawing-buffer size (w×pixelRatio), so a fixed screenshot clip captures only a corner — use `setSize(w,h,true)`.

**→ Leapfrog.** Verified: three consecutive independent Fable PASS verdicts on the final wing (obtuse bend, three shingled rows, rounded tips, layered profile depth, broad-feather read all met); ~450 tris, one draw call (per-row `mergeGeometries` with position-only non-indexed lobes, baked transforms). Residual weaknesses the critics still name (for the NEXT increment before the six-winged seraph array): primary tip-notches could go deeper still; the root/covert band is a touch heavy; interior row separation depends on lighting (near-black material gives nothing in the flat interior — will need in-scene lighting or a subtle per-row tonal/normal break to read the tiers off-silhouette). NOT YET DONE (deferred by the brief): eyes, boss, and the mirror+array into the seraph — this round was the wing SHAPE only.

### L231 — Angel-wing redesign (owner override of L230): the wing is a SMOOTH leading-edge bone + trailing-only feathers + a DOMINANT graduated drooping primary fan; PROPORTION (short arm ~⅓, primaries ~⅔) is the tell, and Fable critics VARY — the human is the arbiter

The L230 wing PASSED three Fable critics but the owner rejected it as a "stiff symmetric feathered boomerang" and specified an ASYMMETRIC, LIVING wing. Rebuilt `buildSeraphWing` around a different grammar (same `featherLobe`/`featherRow`/merge kernel):
- **Leading edge = ONE smooth quadratic-bézier curve** (a thin filled MEMBRANE whose top edge is the bone) flowing THROUGH the bend — NO scallops on top. Feathers hang ONLY off the trailing side (`trailN`), so all scallops are on the bottom edge.
- **A DOMINANT primary fan is the star:** ~5 LONG slim leaf-blades rooted at the wrist, in a tight layered deck, GRADUATED unequal lengths (peak ~2nd feather), each a leaf that SWELLS then tapers to a soft point, each with a real downward ARC, small deterministic ± jitter so no two match.
- **New `featherLobe` params:** `wpeak` (length-fraction of max width → 0.16 broad scallop vs ~0.44 leaf/sabre), `bow` (sideways in-plane curve = the per-blade droop), `rootW` (quill width). These convert the same lobe from a covert scallop into a drooping leaf-feather.

**→ The gotcha that cost the most rounds — PROPORTION, not feather shape.** The single hardest thing to get right, and the last to pass, was the MASS BALANCE: the reference is a SHORT steep arm (~⅓ of the span) with the long primaries DOMINATING (~⅔). Every early version had a long thick arm + a short primary tuft → critics read it as a "broom / palm frond / feather-duster on a stick." Fix: shorten the bézier (bring the wrist close), thin the body wedge FAST toward the wrist (`bodyDepth` tapering to ~0), and make the primaries genuinely long (`pMax` ≈ 1.6× the arm). Also: droop must be a real per-blade ARC (`bow`) — straight blades at fanned angles read as rigid "radial spokes" no matter the angle spread; and the fan must be TIGHT (~20°) with overlapping roots or it reads as a splayed hand, not layered feathers.

**→ Systematize.** LAW for a "living" feathered wing (distinct from L230's symmetric one): smooth membrane leading edge + trailing-only feather scallops + a length-graduated, per-blade-drooping, tightly-overlapped primary fan that DOMINATES the silhouette (~⅔). Judge PROPORTION first (short arm / long primaries), then droop (curved spines), then graduation.

**→ Leapfrog / the meta-lesson.** Fable critics DISAGREE at the margin: the L230 wing got 3 PASSes; a near-identical build in this arc got PASS then FAIL then FAIL from three fresh critics; and across this arc the SAME feature was called "too broad/paddle," then "too thin/needles," then "rods," then "leaves" by different critics. The gate is a strong *forcing function for specifics* (each critic named concrete, actionable geometry), but it is NOT a stable oracle — **the human owner is the final arbiter of feel** (the L8/menu law restated for art). When critics oscillate, stop chasing a unanimous PASS and put the current best in front of the human. NOT YET DONE: eyes, boss, mirror+array into the seraph — still wing-shape only.

### L232 — Shipped the OWNER-AUTHORED wing (supersedes L230/L231): an AUTHORED silhouette (locked bent-spine control points + graduated fan) beats an emergent one; only the T-block is tuned, and the gate test is reduced to the two things that matter

After L230's symmetric wing and L231's asymmetric redesign both drifted under oscillating Fable critics, the owner supplied a compact `buildSeraphWing` and said "use this, don't rewrite the structure — only tune the T block." Its thesis (and the lesson): **AUTHOR the silhouette, don't let it emerge.** The bent two-segment spine (`shoulder`/`wrist`/`tip` control points — arm STEEP, hand OUT) and the graduated primary fan are LOCKED as global structure, so feather-level tweaking cannot lose the proportion or the elbow. Feathers are a single ExtrudeGeometry blade (rounded tip + base) merged; coverts march up the arm, primaries splay off the wrist with a peaked length curve + deterministic `sin(i·2.399)` jitter (no RNG). Returns `{ pivot, dispose }` (mirror = `pivot.scale.x = -1`); the studio was rewired from `.group` to `.pivot`.

**→ The tuning that passed (T-block only, 3 rounds):** defaults FAILED — the primary fan pointed UP nearly parallel to the near-vertical arm, so the elbow read as a smooth diagonal (no bend), and the top primaries bunched near-equal. Fix was pure T-block: **drop the fan angles** (`fanAngLow/High` from 0.30/1.15 → ~0.02/0.88) so the fan sweeps OUT and visibly diverges from the arm (the elbow snaps in); **angle the tip up** (`tip` y 3.0→3.35) to open the front-view elbow from ~L to ~125°; **widen the length contrast** (`primLenMin/Max` 1.7/2.8 → 1.45/3.05) + jitter for graduation. Two consecutive PASS verdicts.

**→ Systematize.** When an emergent generator keeps drifting under a noisy gate, STOP generating and AUTHOR the invariant (here: bend + proportion as fixed control points), exposing only a small `T` tuning block. And reduce the gate to the FEW things that actually define the read — the owner's final test was just two bits: "(A) is the spine visibly bent (arm up, hand out, obtuse elbow) and (B) are the primaries a splayed graduated fan?" A tight 2-clause test converges where a 5-criterion rubric oscillated. NOT YET DONE: eyes, boss, mirror+array into the seraph.
