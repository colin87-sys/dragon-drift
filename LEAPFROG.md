...
### L132 — Headless WebGL renders finally work here: clayshot (geometry) + nimbusshot (lit ground truth), and the strut-camber SIGN bug they caught

**Did / learned.** The human's phone screenshot showed Nimbus's wings looking "fked" — and I had been judging wings on blackout silhouettes, which cannot show shading, camber direction, or material blowout. Two breakthroughs: **(1) headless Chromium WebGL actually rasterizes in THIS environment** — the long-standing "CDN-blocked Playwright, no WebGL in CI" assumption (from the pr-preview era) is FALSE here: `chromium.launch({args:['--use-gl=angle','--use-angle=swiftshader','--enable-unsafe-swiftshader']})` renders the real game. Built `tools/nimbusshot.mjs` (boots the shop, real lit chase-cam capture — the exact pixels the player sees) and `tools/clayshot.mjs`+`clayshot.html` (flat-lit CLAY top/side/3-4 of any PROCEDURAL dragon via buildDragonModel, so geometry is judgeable without finish/lighting noise — the procedural twin of the GLB-only rigshots). **(2) the bug the human's eye caught before any tool did:** the finger-strut camber bowed the WRONG way — `p.z += bowMag` with positive bowMag bows +z (backward), but the leading frame bows −z (forward, `leadEdgePt` = −yAtLead); so struts curved OPPOSITE the frame. Fix: the graded camber value is now NEGATIVE (`camber = -(falloff + (1-falloff)*ease)`) so struts bow forward WITH the frame, magnitude grading leading→inner. The clay top/3-4 renders confirmed the geometry reads cleanly post-fix (convex leading edge, even scallops, coherent body) — proving the in-game "fked" look is dominated by the MEMBRANE MATERIAL blowing out to featureless white under the shop's intense sunset backlight + bloom (an L124/L125 overdraw/bright-surface problem), NOT broken geometry. Darkening the base membrane colour barely helped (environment lighting dominates) — flagged as the next real fix.

**Gotcha.** The shop hero-select turntable is driven by ui.js-internal pointer handlers with no __dd rotate seam and a `#hero-canvas` that `page.$` couldn't box reliably — do NOT try to drag-rotate it for QA; use clayshot for arbitrary-angle geometry reads instead (full camera control, no UI fight).

**→ Systematize.** Wing/'look' work now has a two-view loop: **clayshot** to judge GEOMETRY (camber, proportion, silhouette from any angle, flat-lit) and **nimbusshot/<key>shot** to judge the LIT ground truth (blowout, glow, bloom) — silhouettes remain only for the designcheck laws, never for "does it look right." Any future "it looks wrong" report starts by rendering both, not by reasoning from a phone photo. Camber-direction rule banked: a strut's bow sign must MATCH the leading frame's (both toward −z/forward); a mismatched sign reads as "the strut curves the wrong way."

**→ Leapfrog (innovate).** Next: the membrane blowout (reduce wing SSS/rim under strong backlight, or accept a controlled cloud-glow that doesn't erase the form) — now iterable against real renders; then re-confirm the strut grading with the human on a clayshot 3-4. Generalise clayshot into the standard procedural-dragon QA shot for every hero in the arc. Verified: strut-sign fix inside the opt-in `wingCamberFalloff` branch → shipped roster byte-identical (hull green, fire/water/earth tris unchanged); designgate 16 + face 9 green; designcheck nimbus all green except the standing C1 waiver; tricount shipped rows unchanged.
