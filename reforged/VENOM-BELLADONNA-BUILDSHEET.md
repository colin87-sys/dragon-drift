# BELLADONNA STILETTO — "Venom, patiently brewed" · Premium Build Sheet (fresh wasp-wyrm)
...
- **Sacs = translucent WALL + opaque emissive FILL mesh** — never a translucent glowing volume.
- **Gloss oil-slick dorsal band** — the anti-Vesper finish, diffuse-only, ≤10% coverage.
...
3. **Tone check** — venom drip + needle: confirm it stays elegant-menace, not creepy, on the preview.
4. **Surge name** — "Venom Overdrive" vs "Full Bloom"; wash `[0.07,0.02,0.10]` proposed.
5. **Hindwing phase 0.35** — the hum rate is a feel call; rides the PR preview.

## CHANGELOG
- **v0 (Fable design-director synthesis).** Fresh wasp-wyrm BELLADONNA STILETTO — identity POTENT
  (blacklight apothecary); hero = THE GOSSAMER DOUBLET (four veined glass blades, aux-pivot rig);
  motif = THE VENOM STILL (translucent sacs + opaque emissive fill, 0.05→1.0 BREWING ladder + drip);
  pinched-waist cruciform silhouette (the only concave body break in the roster); UV-orchid 292°
  (deliberately off the mandate's chartreuse — Vesper-eye collision). Next: stub + gate calibration,
  then thorax/waist/gaster → doublet wings (aux-pivot hook) → the still, per-increment Fable gates.
