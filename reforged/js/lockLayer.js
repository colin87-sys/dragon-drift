...
export function updateLockLayer(dt, player, ctx) {
  ...
  if (S.lanceQ.length && !ctx.deflected) {
    ...
    while (S.lanceQ.length && S.lanceQ[0].t <= 0) {
      ...
      ctx.fireLance?.(q.part, q.dmg, q.i, q.n, q.full);   // (i, n) = the wisp's authored fan bearing
    }
  }
...
function releaseVolley(ctx, source) {
  ...
  // song's next grid point — cap auto-release lands ON a beat, a manual tap on
  // the next 16th. Computed by boss.js (releaseQuantDelay) from getBeatClock;
  // absent in a headless/fabricated ctx → 0 → byte-identical launch frames.
  // The volley COMMITS here regardless (locks cleared, lockVolley fired) — a
  // held volley can never be stripped or re-clamped during the wait, and the
  // deflect rule still pauses the queue itself. decay/fork are never held.
  const D = (source === 'cap' ? ctx.gridDelayBeat : source === 'tap' ? ctx.gridDelay16 : 0) || 0;
  const full = S.cap > 0 && pips >= S.cap;
  let i = 0;
  for (const lk of S.locks) {
    for (let s = 0; s < lk.stacks; s++) {
      S.lanceQ.push({ part: lk.part, dmg: dmgEach, t: D + i * (L.lanceStaggerMs / 1000),
        i: i++, n: pips, full, source });
    }
  }
