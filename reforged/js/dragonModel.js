...
import { buildAstralCrownModel } from './astralCrownModel.js'; // bespoke archetype (def.archetype==='astralCrown')
...
export function buildDragonModel(def, opts = {}) {
  ...
  // Bespoke-archetype coexist path: a creature with its OWN from-scratch model
  // (built like the Phoenix was — custom geometry, no shared part-builders) that
  // returns the SAME { group, parts, materials, auraSprite } contract, so the rig
  // and preview drive it unchanged. The procedural roster is untouched.
  if (def.archetype === 'astralCrown') return buildAstralCrownModel(def, opts);
