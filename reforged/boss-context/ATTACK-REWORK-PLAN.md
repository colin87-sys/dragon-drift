...
| 4 | **B-3 THRUMSWARM dials** (SHIPPED) | P2/P3/P4 cadence lower bounds lifted to the 1.2 floor (`[1.2,1.5]`/`[1.2,1.45]`/`[1.2,1.4]`; kept uppers → escalation preserved; extended to P2 which also undercut 1.2) | none | PRESSURE OSTINATO phrase carries the density feel | **Consistency fix, not a difficulty change:** `cadence` is FALLBACK-only for a boss with a phrase machine (`boss.js:2791` uses `rhythmRest` when present), so this aligns the def field with the band floor without touching live pacing. rhythmprint unchanged (min KS 0.26); lifecycle kill ~107.0→107.5s (confirms no pacing effect) |
| 5 | **C.1a STORMREND teach fix** (SHIPPED) | `iris` (slow teach form, gap 0.65) + `secondWave` added to P2 (attacks[] + phrase) | none | both ids shipped | iris no longer debuts inside the dread (law 4 fixed even before the new dread lands). Interim note: P2 is now 4-attack vs P3's 3 — a mild density inversion that resolves when C.1b upgrades P3's dread to the eye-corridor iris-chain (ENG-B). rhythmprint re-cleared (min KS 0.26) |
...
|---|---|---|---|---|
| A | **Per-organ / multi-origin emit** (SHIPPED — crossfire slice): `def.emitOrigins` per-attack-id part map → `resolveEmitOrigins`→`partWorldPos` per emitter, own time-to-impact; `[]`=silent (never post-fallback); `aimVel`→`aimVelFrom` wrapper. `crossfire` live, `aimed`/`stream` wired-dormant. EITHERWING opts in `crossfire` (twins `eitherTwinA/B`). Coexist proven (un-opted headless+live = ±10). No new id. **Deferred:** C.4 dread composition, the `eitherMuzzle` holder volley, other bosses' opt-ins. | `resolveEmitOrigin` + `firePaneRadial` precedents | C.4 (twins), C.6 (lance+chain crossfire), C.7 (bellMouth spiral — E.0.6), C.8 (hands), D (Parallax Regard); ALSO ENG-A-R (reflect-to-part) | **EITHERWING (C.4)** ✓ |
