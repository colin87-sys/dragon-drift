...
| 7 | **`roofline`** — sunken villa | LOW hugger + **scale cue**. step 48 (SHIPPED, Fable gate 4.4) | NEW | **low ribbed terracotta gable just breaking the surface** + antefixae eave-bumps | pitch **~28–32°** (Fable-revised from 15–22°: the shallow Roman pitch read as a FLAT RAFT from the chase camera — steepen so the slope-split reads a roof; ~30° is still Roman-common, not alpine); imbrex ribs downslope; one rafter gap; hip ring w/ atrium hole on big instances | terracotta zone; ladder at the tilted waterline cut |
