...
| 7 | THRUMSWARM | stippled swarm | condenses into YOUR dragon | void-black·star-white + amber focal-eye / scattered points | condenses | claimed | **queen's amber-eye volley → SCATTER STAGGER** (Calamities): parry the eye's amber volley 3× → the queen recoils, the swarm can't re-condense for a 2.5s bonus-damage window (the condensed weak-point stays exposed — parry answers the same condense/scatter puzzle read the fight is built on) |
