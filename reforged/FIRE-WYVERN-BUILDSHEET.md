...
gules driven to char. Hero: **THE UNDERLIT CRESCENT** (the four-fingered bat-fan wing, dark
above, furnace-lit beneath). Motif: **THE STOKE** (the seam network igniting tail-ward).
...
**Rear-chase sentence (rewritten — supersedes the spade/tuck draft):**
*"A charred anvil slung between two vast four-fingered crescents, twin swept horns over one
chipped brow, hind legs splayed bat-wide into the wing–tail gap, and a long ridge-crested
tail rolling end-on to a blunt ember firebrand."*
...
**Distinctiveness gate (vs the shipped roster):**

| Axis | Solar | Phoenix line | Vesper | Tempest | Azure/Ember starters | **Fornax** |
|---|---|---|---|---|---|---|
| Region | top crown | bottom train | lateral scallop | storm-premium FX | compact combs | **ventral UNDERLIGHT + low wide crescent** |
| Limb plan | 6 (dragon) | 6 | 6 | 6 | 6 | **4 — the roster's only true wyvern** |
| Wing | vault-bays + lances | feather ranks | scallop lobes | storm membrane | feather blades | **4-digit bat fan, dominant leading digit, dark top / lit belly** |
...
emissive ramp, CPU-driven, headless-testable — the Vesper `seamRun` plumbing pattern),
terminating in the tail firebrand; the wing membranes backlight from BENEATH; wing tops
stay dark (law 5).
...
- **Bloom:** halo:emitter ≈ **6:1** (ref §7); the halo carries the hue while the core
  clips. Bloom B ≤ 0.5·G; any fringe B ≤ 0.15·G (ref §7) — verify by channel ordering.
...
- **Embers:** **32 cruising (ref §7 24–40) → burst 120 on ignition (ref §7 90–140) →
  hard cap 160** (ref §7). Sparse, chunky, warm; seeded erratic clusters, never a
  metronome (AAA tell #8).
- **Cruise state:** base emissive ≈ 0.04 (imperceptible), `surgeGlowMultiplier` high
  ...
  stay silhouette; `feverEye`, `feverWash`, `surgeHi` all warm-amber). Eyes out of the
  surge arrays.
...
- **Surface — lava-lake plate topology (ref §7):** few long curving seams bounding LARGE
  ...
  spacing ≈ 10× plate thickness. Gen-3 hairlines are shop-turntable-only — say it,
  budget it there. Overlap > weld on every plate joint (house §3.4).
...
bays + 3 knuckles, countable features that survive gameplay distance; a single spar
degenerates to the paper-dart read. All modern wyvern precedent agrees (ref §1 — Weta
rebuilt Smaug onto two limbs so the wings could ACT).
...
- **Camber:** drives off airspeed — **0.14 chord slow ↔ 0.04 chord fast**, deepest sag at
  **40% chord** (ref §5). One dial (`camberMix`) lerped by speed; membrane arcs ≥4
  segments, cup toward the knuckle (house §4.3).
- **Wing-as-arm articulation:** house `wingParts` cascade + medial wrist (`wristT`
  0.2–0.3) + rigid-hand wrist fold + −anchor + outer-wrapper mirror (house §5, non-
...
- **THE STOKE surface:** membrane UNDERSIDE carries the backlight emissive (DoubleSide on
  thin emissive, house §6.5); tops stay in the char value ladder with bay-depth value
  banding (AAA tell #12). Bone digits read as recessed tapered filaments in channels with
  dim skirts (AAA tell #1), never flat bright tape.
...
| seam generations | 0 (grooves only) | 1 | 2 | 3 (gen-3 shop-only) |
...
| embers (cruise/burst) | 0/0 | 12/40 | 24/90 | 32/120 (caps: ref §7) |
...
- **Channel-order probe:** every fire-region pixel R≥G≥B; bloom B ≤ 0.5·G (law 4) —
  a pixel-cross-section check per AAA §4, worst case = warmest sky.
...
- **Ember cap:** ≤160 hard, ≤40 cruise (ref §7).
...
- Overdraw: no additive shells, no enclosing glow mesh (house §1); underside backlight is
  emissive ON the membrane material (already-drawn geometry); one radial-gradient
...
- Particles: 160 hard cap sits inside the 150–300 mobile ceiling (ref §7) — green.
...
- Risk flagged: THE STOKE sweep touches many materials — drive it per-segment on the
  Vesper `seamRun` CPU pattern, not per-pixel shader work.

**Marked turntable-only features (the auditor checks these against the engine):** gen-3
hairline seams · temper-oxide band · eye placement detail (chase cam gets only brow
silhouette) · gape/breath + throat distension (Surge camera + shop) · tail-prop tripod
stance (shop idle). Everything ELSE in this sheet must survive the rear-chase frame.
...
- **Named residuals (human-only, ride the PR preview, each a one-line dial):**
  ...
  5. Chipped-brow read — charming or "broken model"? (delete = one flag).
