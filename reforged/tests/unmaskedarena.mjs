...
check((engaged.igniteLift?.k ?? 0) === 0 && engaged.igniteLift?.glowVis === false && (engaged.igniteLift?.edge ?? 0) === 0 && engaged.igniteLift?.rimShellVis === false && (engaged.igniteLift?.rimGlow ?? 0) === 0,
  `the seraph does NOT ignite in the VOID (igniteK ${engaged.igniteLift?.k}, aura ${engaged.igniteLift?.glowVis}, depth-rim ${engaged.igniteLift?.rimShellVis}) — the gold wreath is heaven-only`);
...
check(heaven.igniteLift && heaven.igniteLift.k > 0.9 && heaven.igniteLift.glowVis === true && heaven.igniteLift.rimEm !== 0 && heaven.igniteLift.wispVis === true && heaven.igniteLift.rimShellVis === true && heaven.igniteLift.rimGlow > 0.2,
  `the seraph IGNITES in the heaven (igniteK ${heaven.igniteLift?.k?.toFixed?.(2)}, rimEm 0x${heaven.igniteLift?.rimEm?.toString(16)}, aura ${heaven.igniteLift?.glowVis}, wisps ${heaven.igniteLift?.wispVis}, depth-rim shell ${heaven.igniteLift?.rimShellVis}/${heaven.igniteLift?.rimGlow})`);
