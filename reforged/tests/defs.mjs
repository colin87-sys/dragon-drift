...
assertEq(sssr.length, 5, 'exactly 5 Eternal-capable (SSSR, procedural) dragons');
...
ok('5 SSSR dragons each carry lanceTint (hex) + lanceRune (key)');
const tints = sssr.map(([, d]) => d.lanceTint);
assertEq(new Set(tints).size, tints.length, 'the 5 Eternal wisp tints are all distinct');
