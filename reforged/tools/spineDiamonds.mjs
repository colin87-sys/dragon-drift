...
// the full ordered column, each diamond cleaned the same way (symmetrized, unit half-extents) → preserves the
// artist's head→tail shape progression when mapped onto the dragon's rows.
const sequence = central.map(canonicalize);
// emit an importable module for the 3D previewer — canonical (single) + the full ordered sequence
writeFileSync(new URL('../js/celestialSpine.js', import.meta.url).pathname,
  `// AUTO-GENERATED by tools/spineDiamonds.mjs — dorsal-spine diamonds TRACED from the stencil\n` +
  `// (each central plate left/right symmetrized + normalized to unit half-extents). Stamp per armour row in\n` +
  `// tools/celestial3D.html, scaled to that row's cell. SPINE_DIAMONDS is the ordered head→tail column. Do NOT hand-edit.\n` +
  `export const SPINE_DIAMOND = ${JSON.stringify(canonical)};\n` +
  `export const SPINE_DIAMONDS = ${JSON.stringify(sequence)};\n`);
console.log(`wrote ../js/celestialSpine.js (${sequence.length} diamonds + canonical)`);
