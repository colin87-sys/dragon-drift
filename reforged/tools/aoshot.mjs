...
writeFileSync('/tmp/ao-off.png', off);
writeFileSync('/tmp/ao-on.png', on);
...
const png = await page.evaluate(async ({ a, b }) => {
  ...
  [[ia, 'prop shading OFF (shipped)'], [ib, 'prop shading ON (N15 baked AO)']].forEach(([im, label], i) => {
    const x = pad + i * (w + pad);
...
writeFileSync('/tmp/ao-montage.png', Buffer.from(png, 'base64'));
await browser.close();
console.log('wrote /tmp/ao-montage.png — judge whether the props are better grounded (dark bases + undersides)');
