...
| I1 | World-suppression grade (§C.2) | ✅ FABLE PASS 4.4/5 (1 revise round) | Killed the magenta wash (postfx `_feverMix` lift term + sat-raise retired; env sky-shift/curtains warm-gated) → subtractive, split-toned world-suppression grade: desat + −0.4 EV (via the single `toneMappingExposure` write, §M.1-4) + vignette, dragon's DARK band (`surgeDark`, new field + wash-hue fallback) pooled in the (1−luma)² shadows, bright band + subject spared. Machine (solar): CORE:DARK **6.5:1** (aim ≥6), world median 0.79× baseline, shadows B>R>G violet (B−R +12.7), no magenta; fire override intact (phoenix sky R−B +75, warm shadows); off-surge byte-identical; `surgefx` envelope asserts green. Revise fixes: split-tone legibility ((1−luma)²), sky value-structure (spare brights), per-dragon capture filenames. **Color-eye proxy pass** (owner is colorblind → Fable stands in for hue taste): verified **colorblind-SAFE** — the transformation reads on VALUE alone (subject/surround luminance ratio 1.28×→2.23×, greyscale-verified; starfield-reveal + vignette are hue-free cues). **Must-fix applied:** suppression DEPTH is now roster-invariant (fire ember sky darkened → sky-drop 0.92×→0.58×, matching the eclipse's 0.43×; `TSKY` metric added) — a gameplay state must read equally strong for every dragon. Taste dials applied: midtone hue reach ((1−luma)^1.5). **Owner residuals (dials):** horizon-band width; long-run comfort; roster differentiation strength. |
| I2.5 | Ambient punch-up + rear-chase cascade + surge-loss + trigger juice | ✅ FABLE PASS 4.4/5 | **Owner-driven** ("barely notice the Surge"). Fable ruled (C) punch-up-first. World-suppression SNAP-with-overshoot (115% by 400ms, settle 900ms) + sustain BREATHE (~0.22Hz, resists adaptation). **Rear-chase cascade REBUILD** (owner caught: eyes face away in chase cam) — 5 dorsal stations travelling TOWARD the camera (crown corona→spine RUSH→wing SPREAD→**tail CRACK**→rim SEAL, onsets 0/135/300/495/725ms), decay rewinds crown-last. Two surge-loss exits: natural slow fade vs damage 2-stutter GUTTER-OUT + world brighten-pop. **Universal trigger JUICE** (the silhouette-dragon fix): slow-mo hitch 0.72×/0.26s + 1-frame flash+bloom+lift + camera kick + gold burst. Machine: `surgefx` 33/33 (cascade order/travel/attack/decay + snap/overshoot + gutter, pure samplers); juice/starters/smoke green; off-Surge byte-identical. Colorblind greyscale gate PASS (tail-crack climax +98 p95 on nearest element; Vesper world 0.64× + wing-rim tell 131→213). **Owner dials/play-verify:** slow-mo hitch depth/frequency (fires often); sustained-state on Vesper 15-20s in; gutter-vs-fade feel; wing-interior value step; far-Vesper rim symmetry. |
| I3 | The beam anatomy (§E) | ✅ FABLE PASS 4.4/5 (1 revise round, 4.0→4.4) | Two-cylinder onion-ring beam → camera-facing RIBBON (cylindrical billboard, witch-hat 3-band cross-section measured `47,95,142,244,250,250,248,191,153,85`, streak flow + 0.4s surge-pulse) + 3-layer muzzle gather (socket→value-lifted petals→white-snap core, radial 242/106/40dn) + land-triggered impact (1-DC Points cluster + ring choreography; legacy 88-sprite bursts DELETED, §M.1-3). Beam life via pure samplers: extend 110ms/1.05 overshoot, incommensurate wobbles (DFT-verified), core-LAST collapse 60/140/225ms. **"Power stays home"** (pre-assess P0): muzzle climax + recoil + taper embraced — muzzle:impact dominance 4.26×. **Revise: the ULTIMATE DUCK** — ambient cascade ×0.55 while the beam owns the frame (boss→game→player→dragon channel, reset-hygiened) — the apex gather now wins its own frame in greyscale. Machine: `surgebeam.mjs` 22 asserts (incl. duck engage/release, DC Δ0 via the `__ddSurgeHide` seam, prewarm). **Owner dials:** wing-spar dashes ≤200dn during ultimate; sustain ring ~210dn cap; tail-streamer duck. **Play-verify:** pulse rhythm, recoil feel, collapse order, duck restore. |
| I4 | Payoff choreography — the ultimate RITUAL (§F) | ✅ FABLE PASS 4.4/5 (1 revise round, 3.7→4.4) | CALL(0.15s)→GATHER(1.15s, repeat 0.65s)→APEX(0.25s)→RELEASE as an authored **conductor**: `surgeRitualScaleAt` owns `game.timeScale` (floor 0.25 at APEX, snap ≥12/s at RELEASE, 1.05 spring overshoot, settle ≤300ms) with priority over slow-mo/cine-slow; ALL ritual/beam clocks on **rawDt** (§M.1-1); i-frames CALL→RELEASE; APEX pose-pin steers `flapPhase` to the high-V (shortest-path, byte-identical off). Stepped GATHER via seeded sub-swell gaussians (3 first cast/2 repeat, uneven by construction); APEX = contrast pocket (convergence STOPS + `surgeApexSilence` on the same edge); RELEASE = `hitstopForce(0.08)` + one capped flash + FOV punch + trauma spike. Trauma² ROTATIONAL camera shake (seeded incommensurate sines, ≤0.9°). **Revise round (7 fixes):** mandala ANATOMY (dark socket ring + 6-wedge rosette + white core, ×1.28 at lock — greyscale edges, not blobs); ritual **HUD duck** (chrome→25%, boss bar stays, latch-verified 0.25 floor + restore); **gather climbs the DRAGON** (`surgeGatherK`→rim +1.6/wings +1.15/body +0.20, damped 14/s); **APEX camera lift** (~1.5u post-lookAt — boss/dragon silhouettes SEPARATE; critic: "the biggest single win", box-art value stack); convergence **LineSegments streaks** (head-bright/tail-dim = inward read in a still, freeze+fade at APEX, hard-cut at RELEASE); impact seam t=0.16 (post-extend); pin wing lift (−0.30·pinW). Machine: `surgejuice.mjs` 20/20 (envelope floor/window/slope/overshoot/settle, stepped gather, repeat APEX+RELEASE byte-identical Δ=0, reduce-fx flash kill, live ritual end-to-end) + surgebeam 22/22 + surgefx 33/33. **Owner dials/play-verify:** convergence streak salience in motion; DODGE-prompt duck exemption (safety vs cinema); rosette petal scale; sun water-stripe at apex; wing-pin height; slow-mo/hitstop feel; audio alignment. |
| I2 | Ignition cascade (§C.3) + sustain/decay (§D.1) | ✅ FABLE PASS 4.2/5 (1 revise round) | Staggered anatomical cascade (eyes→spine→wings→rim) replaces the uniform `surgeMix` blend. Retimed onsets **0/120/265/440ms** (pre-assess caught the plan's 0/120/250/400 fails its OWN ≥10% uneven test) → gaps 126/148/182 measured, uneven ≥15%; attack eye 44ms/rest ≤97ms; reverse decay rim→wings→spine→**eye-last** off remaining feverTimer; 3–4 seeded travelling sustain flares. Colorblind gate (greyscale story test) 3/4 beats legible. Revise fixes: eye-corona carrier (subpixel eye can't read at chase distance), rim luminance signature (full>wings), reclaim contrast (gate halo/body/core by cascade + armed-dim resting stations), pinned-pose capture. Machine: `surgefx` 25/25 (order/gaps/attack/decay/flares via pure envelope samplers), off-Surge byte-identical, mulberry32 (no Math.random). **PRIME deferred** (needs a predictive surge-imminent signal; not in the §I-5/7 gate). **Owner decision surfaced:** the bright-gold hero (Solar Sovereign) STATIC gold lances cap the dark→light drama at ~2/3 — the cascade reads most dramatically on a dark/silhouette hero (Vesper/Wraith); hero-choice, not a defect. **Owner dials:** ignition loudness (master mult); eye-corona decay length; wings-beat salience (a t≈0.45 wing-spread pose — I4 scope); sustain-breathing depth. |
